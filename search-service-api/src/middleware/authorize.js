...
// Try to load User and Role models for DB-driven permissions
let User = null;
let Role = null;
try {
  // Search service doesn't have its own User/Role models, but can query shared DB
  const mongoose = require('mongoose');
  User = mongoose.model('User');
  Role = mongoose.model('Role');
} catch (e) {
  // Models not available, will use static permissions only
}
    ...
    try {
      ...
      if (User) {
        try {
          ...
          // If user lookup fails, use role from token
          console.warn('Failed to fetch user from DB, using token role:', error.message);
        }
      }

      // Use role from request (either from DB or token)
...
const authorizeAll = (requiredPermissions) => {
  return async (req, res, next) => {
    try {
      ...
      // Fetch latest user info if User model available
      if (User) {
        try {
          const latestUserInfo = await User.findById(req.user._id).lean();
