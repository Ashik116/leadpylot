...
/**
 * Authorization middleware - user must have at least one of the given permissions
 * @param {string[]} allowedPermissions - Array of permission keys (any one grants access)
 * @returns {function} - Express middleware function
 */
const authorizeAny = (allowedPermissions) => {
  return async (req, res, next) => {
    try {
      if (!req.user || !req.user._id) {
        return res.status(401).json({ error: 'Authentication required' });
      ...
      for (const permission of allowedPermissions) {
        const permitted = await hasPermission(userRole, permission);
